...
**規劃不在 tick 的關鍵路徑上。** 規劃 agent 要讀完整個 repo 才判斷得出誰會撞誰，
實測 25 個任務跑 13 分鐘；而 run() 是 `await tick()` 之後才 sleep，await 它就等於
那十幾分鐘內一輪 tick 都不發生——PR 沒人看審查、核准的沒人合併、卡住的沒人重派。
改成 per-repo 背景執行（旗標防重複發動、finally 必定放旗標），
代價是新任務要等下一輪才派得出去（預設 30 秒），換掉的是十幾分鐘的全面停擺。
