...
export class PlanAgent {
  ...
  private async runQuery(prompt: string, cwd: string, repo?: string, signal?: AbortSignal): Promise<string> {
    ...
    // 規劃者是**唯一一次看到整批任務**的角色：「兩張卡的要求彼此矛盾」只有它看得見
...
export function buildPlanPrompt(tasks: Task[], inFlight: InFlightGroup[] = [], previousError = ''): string {
  ...
  p.push(
    `\n## 判準（**有優先順序**：由上往下套，先命中的那一條勝出）\n` +
      ...
      `\n1. **類似的、相關的、屬於同一件事的任務 → 放同一群。**（最高優先）\n` +
      `   \n` +
      `   「類似」由你判斷，不要只看檔案。常見的訊號（有任何一個就算）：\n` +
      `   · **docRefs 指到同一份文件**——那代表它們本來就是一件事被拆成多張卡，這是最強的訊號；\n` +
      `   · 同一種修法套用在不同地方（例如「把觸控熱區補到 44×44」出現在六個元件）；\n` +
      `   · 同一個功能／同一塊介面／同一個主題的不同部分；\n` +
      `   · 你讀完覺得「這幾張卡本來就是同一件事」。\n` +
      `   \n` +
      `   **這一條和檔案完全沒有關係。** 動到完全不同的檔案照樣要合在一起。\n` +
      `   把類似的事拆成好幾個 PR 沒有任何好處：同樣的判斷要重複做好幾次、\n` +
      `   做法還可能不一致，而且每一個都要各自跑一輪完整驗證。\n` +
      `2. **會動到同一個檔案的任務 → 放同一群。**\n` +
      `   在 taskIds 裡照該做的先後排好即可。只有第 3 條完整命中才准拆開。\n` +
      `3. **可以「共用檔案卻拆成兩群」的唯一例外**——下面兩點要**同時**成立：\n` +
      ...
      `4. **同一階段（可並行）的唯一條件：兩群的 files 完全沒有交集。**\n` +
      `   只要有一個檔案同時出現在同一階段的兩個群，那兩群就不准放同一階段——\n` +
      `   不是併成一群（第 2 條），就是拆到不同階段（第 3 條）。\n` +
