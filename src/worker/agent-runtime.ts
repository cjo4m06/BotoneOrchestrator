...
export const ASK_HUMAN_TOOL_DESCRIPTION =
  ...
  '② **修復點不在你碰得到的範圍**：別人的程式、你沒有的權限、需要人拍板的規格決策\n' +
  '③ 你**已經試過至少一次而失敗**——同一個障礙第二次擋住你，就開口\n\n' +
  '**判準是「我修不修得動」，不是「後果嚴不嚴重」。**\n' +
  '改一行 CSS 可逆又低風險，但如果紅燈的根因在調度器程式裡、或要改的檔案不在你的工作區，' +
  '你碰不到——那要問。\n\n' +
  '**需求互斥也算**：如果兩條（或多條）要求在物理上無法同時成立，說明衝突點讓人決定誰讓步，' +
  '**不要自己選一邊**——你選的那一邊會在下一輪被另一邊退回，來回燒掉好幾輪。\n\n' +
