...
/**
 * ask_human 的門檻。**判準是「能力」，不是「後果」。**
 *
 * ── 為什麼換軸 ──
 *
 * 舊版寫「僅在遇到『不可逆或高風險』且無法從規格/程式碼判斷的歧義時提問」。
 * 「可逆／高風險」講的是**後果**，但真正該問的是**我修不修得動**。
 *
 * 實跑（zZb5MGTMdQRZ，2026-08-04）：DoD 三條在幾何上互斥，唯一出路是「改 spec 記載特例」，
 * 而 spec/ 不在 repo、docs MCP 唯讀——agent 寫不進去。它改的是一行 CSS：可逆、低風險、
 * 沒有歧義，**照舊版門檻完全不該問**。於是它撞牆四輪、燒掉 36 美元，最後那個 PR 帶著
 * 一條沒滿足的 DoD 被人按核准合併，而「這裡是刻意的特例」沒有被記在任何地方。
 *
 * 同一個「spec 寫不進去」在 14 小時前已經害過另一個任務（QTa5wZ5CIi_z）。
 *
 * ── 為什麼拆成 A／B 兩條路 ──
 *
 * 換軸只救了一半。舊版三條是 **AND**，而第一條是「實際擋住交付」——
 * 那救得到「DoD 關卡亮紅燈、我交不出去」，救不到**閘門是綠的**那半邊。
 *
 * 壞掉的規格的特徵正好是：它不擋你產出 diff，只讓你產出**錯的**東西。
 * 於是第一條永遠不成立，AND 一擋，每個「規格本身有問題」都被導去 report_friction，
 * agent 照著錯的規格實作、留一筆沒人看的紀錄。實跑三筆（2026-08-05～06，正式庫）：
 *   · #956 它量了兩個房型、做 A/B 對照、還用改房名反證，**推翻了 DoD 的前提**，
 *     然後照著它已經證偽的前提交出局部修——因為 DoD 剛好滿足得了。
 *   · #893 規格與 DoD 互相矛盾，它**自己選了 DoD 那一邊**出貨，理由只寫在 Vue 模板註解裡。
 *   · #1072 程式碼是對的、只有文件過時——這筆才是 report_friction 的正確用法。
 * 三筆都完全照規則做了。所以要改的是規則，不是 agent。
 *
 * B 那條**刻意不要求「已經試過一次」**：自相矛盾的規格是試不出結果的，
 * 多試一輪只是多燒一輪然後選同一邊。
 *
 * ── 反例是必要的，不是裝飾 ──
 *
 * 只寫「該問什麼」會讓 agent 在每個猶豫的岔路口都開口，而 ask_human 會 park **整個群組**
 *（不只這張卡）。所以「不該問什麼」要用同樣的篇幅寫清楚。
 */
export const ASK_HUMAN_TOOL_DESCRIPTION =
  '停下來問人。**兩種情況要問。**\n\n' +
  '**A. 有東西擋住你，而你修不動。** 三件事同時成立才問：\n' +
  '① 它**實際擋住交付**——不是「有點礙事」「我不太確定」\n' +
  '② **修復點不在你碰得到的範圍**：別人的程式、你沒有的權限\n' +
  '③ 你**已經試過至少一次而失敗**——同一個障礙第二次擋住你，就開口\n' +
  '判準是「我修不修得動」，不是「後果嚴不嚴重」：改一行 CSS 可逆又低風險，' +
  '但如果紅燈的根因在調度器程式裡、或要改的檔案不在你的工作區，你碰不到——那要問。\n\n' +
  '**B. 你交得出東西，但不知道交的對不對。** 這三種**一發現就問，不必先試一次**：\n' +
  '· 規格自相矛盾，或規格與這張卡的 DoD 互相打架\n' +
  '· 你實測推翻了規格／DoD 的前提\n' +
  '· 兩條要求互斥，在物理上無法同時成立\n' +
  '**「我還交得出 diff」不是繼續做的理由。** 規格壞掉的時候你永遠交得出某個 diff，' +
  '只是不知道對不對——那正是要問的時候。**不要自己選一邊**：你選的那邊會在下一輪被' +
  '另一邊退回，而且沒有人會知道你選過。\n\n' +
  '**這些不要問**，自己拿主意並記在總結的「假設」裡：\n' +
  '· 改法不確定、兩個方案都可行要選一個\n' +
  '· 測試偶爾紅、可逆的樣式決定\n' +
  '· 你還沒動手試過的事\n\n' +
  '**代價要知道**：問了會把**整個群組**停下來等人回覆，不只你這張卡。';
      ...
      tools: [
        ...
        tool(
          ...
          FRICTION_TOOL_DESCRIPTION,
          {
            ...
            // 這裡刻意沒有 blocked。理由見 friction.ts 的說明：那個值沒有任何消費端，
            // 是一條「回報成功但什麼都不會發生」的安慰劑升級管道。schema 留著它的話，
            // agent 照樣會被問、照樣會誠實填，然後答案在 parse 時被丟掉。
          },
          createFrictionHandler(this.deps.frictionSink ?? NOOP_SINK, this.log, taskId, 'coder'),
        ),
        tool(
          'ask_human',
          ASK_HUMAN_TOOL_DESCRIPTION,
          {
            question: z.string(),
...
export function buildAgentPrompt(input: IterateInput): string {
  ...
  p.push(
    `\n## 完成要求\n` +
      ...
      `- 可逆的小決定自行採合理預設並記錄假設。\n` +
      `- **判準是「我修不修得動」，不是「後果嚴不嚴重」。** 實際擋住你、修復點又不在你碰得到的\n` +
      `  範圍（別人的程式、你沒有的權限、要人拍板的規格決策），而且你已經試過一次還是過不去\n` +
      `  ——用 ask_human。**同一個障礙第二次擋住你就開口。**\n` +
      `- **兩條要求互斥時說出來，不要自己選一邊。** 你選的那一邊會在下一輪被另一邊退回。\n` +
