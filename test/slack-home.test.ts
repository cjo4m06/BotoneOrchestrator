...
describe('App Home 版面', () => {
  ...
  it('顯示進行中的工作、用人看得懂的詞、附經過時間', () => {
    ...
    // 絕對時間是刻意的：這一頁不會自己更新，凍住的「已 6 分」會騙人
    // （顯示 6 分其實已經 40 分），凍住的「14:05 起」永遠是對的
    assert.match(s, /\d{2}\/\d{2} \d{2}:\d{2} 起/, '要有絕對的開始時間');
    assert.match(s, /已 6 分 12 秒/, '相對時間仍然有用，但只是輔助');
    assert.match(s, /第 2\/2 次嘗試/, '重問也要看得到——一次嘗試就是好幾分鐘');
  });

  /** 不講的話，人會以為畫面是即時的——而它其實是一張照片。 */
  it('標題就講明這是快照、不會自己更新', () => {
    assert.match(textOf(homeViewBlocks(home())), /不會自己更新/);
  });
