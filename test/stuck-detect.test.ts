...
describe('ask_human 的門檻：軸是「能力」不是「後果」', () => {
  ...
  /**
   * B 路徑。舊版三條是 AND，第一條「實際擋住交付」擋掉了整類壞規格——
   * 壞規格不擋你產出 diff，只讓你產出錯的東西，所以第一條永遠不成立。
   * 正式庫三筆（#956 推翻 DoD 前提仍出貨、#893 自己選一邊、#1072）全是這樣來的。
   */
  it('B 路徑：交得出東西但不知道對不對，也要問——而且不必先試一次', () => {
    assert.match(d, /不知道交的對不對/);
    assert.match(d, /規格自相矛盾/);
    assert.match(d, /推翻了規格／DoD 的前提/);
    assert.match(d, /不必先試一次/, '自相矛盾的規格試不出結果，多試一輪只是多燒一輪');
    assert.match(d, /「我還交得出 diff」不是繼續做的理由/, '這句是分界線，拿掉就退回舊行為');
  });
